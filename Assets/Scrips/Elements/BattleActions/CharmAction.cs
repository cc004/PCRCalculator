  ...
  public class CharmAction : ActionParameter
  {
      ...
      Dictionary<eValueNumber, FloatWithEx> _valueDictionary)
    {
      base.ExecAction(_source, _target, _num, _sourceActionController, _skill, _starttime, _enabledChildAction, _valueDictionary);
            double pp = (double)_valueDictionary[eValueNumber.VALUE_3] * (double)BattleUtil.GetDodgeByLevelDiff(_skill.Level, _target.GetLevel());
            if ((double) BattleManager.Random(0.0f, 1f,
                new PCRCaculator.Guild.RandomData(_source, _target.Owner, ActionId, 7, (float)pp)) < (double) _valueDictionary[eValueNumber.VALUE_3] * (double) BattleUtil.GetDodgeByLevelDiff(_skill.Level, _target.GetLevel()))
      {
        this.AppendIsAlreadyExeced(_target.Owner, _num);
        UnitCtrl.eAbnormalState _abnormalState = UnitCtrl.eAbnormalState.CONVERT;
        switch ((CharmAction.eEffectType) this.ActionDetail1)
        {
          case CharmAction.eEffectType.CONVERT:
            _abnormalState = UnitCtrl.eAbnormalState.CONVERT;
            break;
          case CharmAction.eEffectType.CONFUSION:
