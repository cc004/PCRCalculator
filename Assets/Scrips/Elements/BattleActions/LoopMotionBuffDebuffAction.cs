...
namespace Elements
{
  public class LoopMotionBuffDebuffAction : ActionParameter
  {
    ...
    {
      triggered = true;
      for (int index = 0; index < targetAndValueList.Count; ++index)
      {
        BuffDebuffData targetAndValue = targetAndValueList[index];
        targetAndValue.Target.EnableBuffParam(targetAndValue.BuffParamKind, targetAndValue.Value, false, _source, ActionDetail1 % 10 != 1, false,90);
      }
    }
