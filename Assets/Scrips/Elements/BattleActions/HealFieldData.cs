...
namespace Elements
{
  ...
  {
    public HealFieldData.eValueType ValueType { get; set; }

    public HealFieldData.eEffectType EffectType { get; set; }
    ...
    public override void OnRepeat()
    {
      ...
      for (int count = this.TargetList.Count; index < count; ++index)
                this.TargetList[index].Owner.SetRecovery((int)this.Value, this.IsMagic ? UnitCtrl.eInhibitHealType.MAGIC : UnitCtrl.eInhibitHealType.PHYSICS, this.TargetList[index].Owner, UnitCtrl.GetHealDownValue(this.PPOJKIDHGNJ), _target: this.TargetList[index], _releaseToad: true);
