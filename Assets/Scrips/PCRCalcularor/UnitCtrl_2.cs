    ...
    public partial class UnitCtrl : MonoBehaviour
    {
        ...
        internal int SetDamage(DamageData damageData, bool byAttack, int actionId, ActionParameter.OnDamageHitDelegate onDamageHit, bool hasEffect, Skill skill, bool energyAdd, Action onDefeat, bool noMotion, float weight, float actionWeightSum, Func<int, float, int> p)
        {
                    ...
                    if (damageData.ActionType != eActionType.ATTACK_FIELD && (skill == null || skill.IsLifeStealEnabled))
                    {
                        ...
                        if (lifesteal >= 1)
                        {
                            ...
                            if (recovery_value > 0)
                            {
                                ...
                                damageData.Source.SetRecovery((int)recovery_value, eh, damageData.Source, false, false,
                                    false,//鸽了
                                    false, true, null);
                            }
                        }
            ...
            if (skill != null)
            {
                ...
                if (damage >= 1)
                {
                    if (!skill.DamagedPartsList.Contains(damageData.Target))
                    {
                        skill.DamagedPartsList.Add(damageData.Target);
                    }
                }
            }
            if (damageData.Source != null)
        ...
        internal int SetDamageImpl(DamageData damageData, bool byAttack, ActionParameter.OnDamageHitDelegate onDamageHit, bool hasEffect, Skill skill, bool energyAdd, bool critical, Action onDefeat, bool noMotion, Func<int, float, int> p)
        {
            ...
            if (hasEffect)
            {
                ...
                if (isGuildEnemy)
                {
                    BattleUIManager.Instance.ReflashGuildEnemyTotalDamage(damage);
                }
            }
            ...
            if (!HasUnDeadTime)
            {
                ...
                if (Hp <= 0 && !isDead && actionState <= eActionState.DAMAGE)
                {
                    ...
                    {
                        if (damageData.Source.IsAbnormalState(eAbnormalState.FEAR))
                        {
                            KillBonusTarget = null;
                        }
                        else
                        {
                            KillBonusTarget = damageData.Source;
                        }
                        onDefeat?.Invoke();
        ...
        private void EnableAbnormalState(eAbnormalState abnormalState, bool enable, bool reduceEnergy = false, bool _switch = false)
        {
            ...
            switch (abnormalState)
            {
                ...
                case eAbnormalState.DECOY:
                    UnitCtrl decoyTarget = IsOther ? staticBattleManager.DecoyOther : staticBattleManager.DecoyUnit;
                    if (enable)
                    {
                        if(decoyTarget!= null)
                        {
                            if(decoyTarget != this)
                            {
                                decoyTarget.EnableAbnormalState(eAbnormalState.DECOY, false);
                            }
                        }
                        if (isOther)
                        {
                            staticBattleManager.DecoyOther = this;
                        }
                        else
                        {
                            staticBattleManager.DecoyUnit = this;
                        }
                    }
                    else
                    {
                        if(decoyTarget == this)
                        {
                            if (isOther)
                            {
                                staticBattleManager.DecoyOther = null;
                            }
                            else
                            {
                                staticBattleManager.DecoyUnit = null;
                            }
                        }
                    }
                    break;
            ...
            string word = UnitName + (enable?"获得异常状态":"失去异常状态") + "<color=#C11900>" + abnormalState.GetDescription() + "</color>";
            BattleUIManager.Instance.LogMessage(word, eLogMessageType.BUFF_DEBUFF, IsOther);
        ...
        private IEnumerator UpdateHpRegeneration(int _regeneId)
        {
            ...
            while (true)
            {
                if (IsAbnormalState(eAbnormalState.REGENERATION))
                {
                    if(_regeneId == currentHpRegeneId)
                    {
                        time_5 += DeltaTimeForPause_BUFF;
                        if (time_5 <= 1)
                        {
                        ...
                        time_5 = 0;
                        AbnormalStateCategoryData categoryData = abnormalStateCategoryDataDictionary[eAbnormalStateCategory.REGENERATION];
                        eInhibitHealType t = categoryData.SubValue != 1 ? eInhibitHealType.MAGIC : eInhibitHealType.PHYSICS;
                        SetRecovery((int)categoryData.MainValue, t, categoryData.Source, true, false, false, true, true);
                        yield return null;
                        continue;
                    }
                }
            ...
            while (true)
            {
                if (IsAbnormalState(eAbnormalState.REGENERATION))
                {
                    if (_regeneId == currentHpRegeneId)
                    {
                        ...
                        time_5 = 0;
                        AbnormalStateCategoryData categoryData = abnormalStateCategoryDataDictionary[eAbnormalStateCategory.REGENERATION];
                        ChargeEnergy(eSetEnergyType.BY_USE_SKILL, categoryData.MainValue, true, this, true, false, true, false);
                        yield return null;
                        continue;
                    }
                }
