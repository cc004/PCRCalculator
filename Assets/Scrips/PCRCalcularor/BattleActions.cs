...
namespace PCRCaculator.Battle
{
    ...
    public class ActionParameter
    {
        ...
        {
            int index = skill.ActionParameters.FindIndex(a => a == this) + 1;
    ...
    public class MoveAction : ActionParameter
    {
        ...
        private static Vector3 CalculatePosotion(UnitCtrl _source, BasePartsData _target, UnitActionController _sourceActionController, Dictionary<int, float> _valueDictionary)
        {
            ...
            bool isBack = vector3.x > 0 ? false : true;
            float distance = Mathf.Max(0, Mathf.Abs(vector3.x) - _target.GetBodyWidth() / 4 - x);
        ...
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            endAction = _skill.ActionParameters.Find(a => a.ActionId == this.ActionDetail2);
        }
        ...
        {
            bool isRight = _source.transform.position.x < _skill.OwnerReturnPosition.x;
            while (true)
            {
                float deltaX = _velocity * Owner.DeltaTimeForPause;
                if (!isRight)
                {
                    deltaX *= -1;
                }
                _source.SetPosition(deltaX);
                float distance = _skill.OwnerReturnPosition.x - _source.transform.position.x;
                if (!isRight)
                {
                    distance *= -1;
                }
            ...
            _source.transform.position = _skill.OwnerReturnPosition;
            _source.PlaySkillAnimation(_skill.SkillId, 4, false);
            _source.StartCoroutine(moveType4ReturnEnd(_source, _skill));
            //BattleUIManager.Instance.LogMessage("出现异常：这部分代码暂时鸽了！",eLogMessageType.ERROR,_source.IsOther);
        }
    ...
    public class DamageChargeAction : ActionParameter
    {
        ...
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            ...
            if (ActionDetail2 != 0)
            {
                targetAction = _skill.ActionParameters.Find(a => a.ActionId == ActionDetail2);
        ...
        private IEnumerator PlayMotionLoopForDamageCharge(eAnimationType animeId, Skill skill, UnitCtrl _source, UnitActionController _sourceActionController, Skill _skill)
        {
            ...
            while (true)
            {
                ...
                //播放第二段动画
                _source.PlaySkillAnimation(_skill.SkillId, 2, true);
                //特效
                //结束时特效
                break;
            }
        ...
        {
            Value[1] = MasterData_mine.values[0] + MasterData_mine.values[1] * level;
            /*var group = Elements.Battle.MyGameCtrl.Instance.tempData.SettingData.GetCurrentPlayerGroup();
            if (group.isSpecialBoss && group.specialBossID == 666666)
            {
    ...
    public class DecoyAction : ActionParameter
    {
        public override void ExecAction(UnitCtrl source, BasePartsData target, int num, UnitActionController sourceActionController, Skill skill, float starttime, Dictionary<int, bool> enabledChildAction, Dictionary<int, float> valueDictionary)
        {
            base.ExecAction(source, target, num, sourceActionController, skill, starttime, enabledChildAction, valueDictionary);
            AppendIsAlreadyexeced(target.Owner, num);
            target.Owner.SetAbnormalState(source, eAbnormalState.DECOY, valueDictionary[1], this, skill, 0, 0, false, 0);
        }
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            base.ExecActionOnStart(_skill, _source, _sourceActionController);
    ...
    public class GiveValueAction : ActionParameter, ISingletonField
    {
        ...
        protected virtual void CreateValue(UnitCtrl _source, Skill _skill, Dictionary<int, float> _valueDictionary, Dictionary<int, float> _addValue, int _evalue, BasePartsData _target)
        {
            switch (_valueDictionary[1])//只有1，2，4会被运行到
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    _addValue[_evalue] = _skill.DamagedPartsList.Count * _valueDictionary[2];
                    break;
                case 6:
        ...
        public override void ExecAction(UnitCtrl source, BasePartsData target, int num, UnitActionController sourceActionController, Skill skill, float starttime, Dictionary<int, bool> enabledChildAction, Dictionary<int, float> valueDictionary)
        {
            ...
            //log
            ActionParameter action = skill.ActionParameters.Find(a => a.ActionId == ActionDetail1);
            SetValue(dic, action);
        }
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            ...
            if (Value[1] == 2)
            {
                if (_skill.ActionParameters.Count >= 1)
                {
                    ActionParameter actionParameter = _skill.ActionParameters[0];
                    foreach (ActionParameter parameter in _skill.ActionParameters)
                    {
    ...
    public class IfForAllAction : ActionParameter
    {
            ...
            if (ActionDetail1 != 1000)//33，1000，1211
            {
                if (ActionDetail1 == 33 || ActionDetail1 == 1211)
                {
                    ...
                    if (k)
                    {
                        ...
                        ActionParameter action = skill.ActionParameters.Find(a => a.ActionId == ActionDetail3);
                    ...
                    else
                    {
                        ...
                        ActionParameter action = skill.ActionParameters.Find(a => a.ActionId == ActionDetail2);
                        CancelAction(action, skill);
                    }
                    sourceActionController.StartCoroutine(sourceActionController.UpdateBranchMotion(this, skill));
                    return;
        ...
        public void B_0()
        {
            ...
            if (ActionDetail3 != 0)
            {
                skill_b_0.ActionParameters.Find(a => a.ActionId == ActionDetail3).CancelByIfForAll = true;
            }
        }
        public void B_2()
        {
        ...
        public void B_3()
        {
            if (ActionDetail2 != 0)
            {
                skill_b_0.ActionParameters.Find(a => a.ActionId == ActionDetail2).CancelByIfForAll = true;
            }
    ...
    public class EnchantLifeStealAction : ActionParameter
    {
        public override void ExecAction(UnitCtrl source, BasePartsData target, int num, UnitActionController sourceActionController, Skill skill, float starttime, Dictionary<int, bool> enabledChildAction, Dictionary<int, float> valueDictionary)
        {
            ...
            if (valueDictionary[3] >= 1)
            {
                for (int i = (int)valueDictionary[3]; i > 0; i--)
                {
                    queue.Enqueue((int)valueDictionary[1]);
                }
            }
            target.Owner.LifeStealQueueList.Add(queue);
            target.Owner.OnChangeState?.Invoke(target.Owner, eStateIconType.BUFF_ADD_LIFE_STEAL, true);
        }
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            base.ExecActionOnStart(_skill, _source, _sourceActionController);
    ...
    {
        private List<UnitCtrl> targetListDefault;
        public override void ExecAction(UnitCtrl _source, BasePartsData _target, int _num, UnitActionController _sourceActionController, Skill _skill, float _starttime, Dictionary<int, bool> _enabledChildAction, Dictionary<int, float> _valueDictionary)
        {
        ...
        private IEnumerator UpdateIdle(UnitCtrl _source)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    //_source.IsMoveSpeedForceZero = false;
                    _source.StartCoroutine(WaitCancelSpeedZero(_source));
                    _source.IdleOnly = false;
                    //_source.FCastTimer = 0;
                    _source.TargetEnemyList = targetListDefault;
                    _source.SetState(eActionState.WALK, 0);
                    yield break;
                }
            }
        }
        private IEnumerator WaitCancelSpeedZero(UnitCtrl source)
        {
            float time_5_2 = 2.5f;
            while (time_5_2 > 0)
            {
                time_5_2 -= source.DeltaTimeForPause;
                yield return null;
            }
            source.IsMoveSpeedForceZero = false;
        }
    ...
    public class RatioDamageAction : ActionParameter
    {
        ...
        {
            base.ExecAction(_source, _target, _num, _sourceActionController, _skill, _starttime, _enabledChildAction, _valueDictionary);
            _target.UbAttackHitCount1++;
            int num1 = base.ActionDetail1;
            if (!base.HitOnceDic.ContainsKey(_target))
            {
                base.HitOnceDic.Add(_target, false);
                base.HitOnceKeyList.Add(_target);
            }
            if (!base.HitOnceDic[_target])
            {
                this.targetCurrentHps[_target] = (long)_target.Owner.Hp;
            }
            int num = 0;
            switch (((eTargetParameter)base.ActionDetail1))
            {
                case eTargetParameter.MAX_HP:
                    num = Mathf.FloorToInt((((((float)_target.Owner.BaseValues.Hp) * _valueDictionary[1]) / 100f) * base.ActionExecTimeList[_num].Weight) / base.ActionWeightSum);
                    break;

                case eTargetParameter.CURRENT_HP:
                    num = Mathf.FloorToInt((((((float)this.targetCurrentHps[_target]) * _valueDictionary[1]) / 100f) * base.ActionExecTimeList[_num].Weight) / base.ActionWeightSum);
                    break;
            }
            DamageData data1 = new DamageData();
            data1.Target = _target;
            data1.Damage = num;
            data1.DamageType = (eDamageType)base.ActionDetail2;
            data1.Source = _source;
            //data1.DamageSoundType = DamageData.eDamageSoundType.HIT;
            data1.DamageEffectType = base.ActionExecTimeList[_num].DamageNumType;
            data1.DamageNumScale = base.ActionExecTimeList[_num].DamageNumScale;
            data1.IgnoreDef = true;
            data1.LifeSteal = _source.IsPartsBoss ? 0 : ((int)_source.GetLifeStealZero());
            data1.ActionType = eActionType.RATIO_DAMAGE;
            DamageData data = data1;
            ActionParameter.OnDamageHitDelegate onDamageHit = base.OnDamageHit1;
            Skill skill = _skill;
            if (_target.Owner.SetDamage(data, true, base.ActionId, onDamageHit, true, skill, true, null, false, base.ActionExecTimeList[_num].Weight, base.ActionWeightSum, null) != 0)
            {
        ...
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            base.ExecActionOnStart(_skill, _source, _sourceActionController);
            /*
            this.parts = _source.BossPartsListForBattle.Find(delegate (PartsData e) {
                return e.Index == _skill.ParameterTarget;
            });
        ...
        {
            base.ReadyAction(_source, _sourceActionController, _skill);
            for (int i = 0; i < base.HitOnceKeyList.Count; i++)
            {
                base.HitOnceDic[base.HitOnceKeyList[i]] = false;
            }
        }
    ...
    public class RegenerationAction : ActionParameter
    {
        ...
        public override void ExecAction(UnitCtrl source, BasePartsData target, int num, UnitActionController sourceActionController, Skill skill, float starttime, Dictionary<int, bool> enabledChildAction, Dictionary<int, float> valueDictionary)
        {
            ...
            if (ActionDetail1 == 1)
            {
                if(source.IsPartsBoss)
                    Debug.LogError("咕咕咕！");
                strValue = source.GetAtkZero();
            }
            else
            {
                strValue = source.GetMagicStrZero();
            }
            if(ActionDetail2 == 2)
            {
                target.Owner.SetAbnormalState(source, eAbnormalState.TP_REGENERATION, valueDictionary[5], this, skill, valueDictionary[1], ActionDetail1, false, 0);
            }
            if(ActionDetail2 == 1)
            {
                int valueint = (int)(valueDictionary[1] + valueDictionary[3] * strValue);
                target.Owner.SetAbnormalState(source, eAbnormalState.REGENERATION, valueDictionary[5], this, skill, valueint, ActionDetail1, false, 0);
            }
        }
        public override void ExecActionOnStart(Skill _skill, UnitCtrl _source, UnitActionController _sourceActionController)
        {
            base.ExecActionOnStart(_skill, _source, _sourceActionController);
            parts = _source.DummyPartsData;
        }
        public override void SetLevel(float level)
        {
            Value[1] = MasterData_mine.values[0] + MasterData_mine.values[1] * level;
            Value[3] = MasterData_mine.values[2] + MasterData_mine.values[3] * level;
            Value[5] = MasterData_mine.values[4] + MasterData_mine.values[5] * level;
